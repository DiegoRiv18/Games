...
def make_rectangle(canvas, bottom_right, width, height, color="#3D9970", tag=None):
    x, y = bottom_right
...
def make_line(canvas, coordinates, curvy=False, width=2, tag=None):
    canvas.create_line(
        ...
        smooth=curvy,
        tag=tag)
def make_eye_circles(canvas, center, radius_x, radius_y, size, color='#FF4136', tag=None, stroke_width=1, outline=None):
    make_poly_oval(canvas,(center[0] - size/3.7,center[1] - size/6),radius_x,radius_y,color = 'black', tag = 'left')
    make_poly_oval(canvas,(center[0] + size/3.7,center[1] - size/6),radius_x,radius_y,color = 'black', tag = 'right')
    rotate(canvas,'left', 150, (center[0] - size/3.7,center[1] - size/6))
    rotate(canvas,'right',30, (center[0] + size/3.7,center[1] - size/6))

def make_nose(canvas, center, size=10,nose_color:str='black', tag = None):
    canvas.create_polygon(
    [ (center[0]-size, center[1]),  (center[0]+size, center[1]), (center[0], center[1]+1.25*size) ],
    fill=nose_color,
    smooth=True, tag = tag)
